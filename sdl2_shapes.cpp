...
struct Deleter {
	template <class T> void operator()(T* o) const noexcept { CDeleter(o); }
};
...
struct Renderer {
	explicit Renderer(Window& window)
		: impl_(
			SDL_CreateRenderer(window.handle(), -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC)
		)
	{
		if (!impl_)
			throw std::runtime_error(SDL_GetError());
	...
	void setColor(Color c) {
		SDL_SetRenderDrawColor(impl_.get(), c.color.r, c.color.g, c.color.b, c.color.a);
	}

	Color getColor() const {
		Color c;
		SDL_GetRenderDrawColor(impl_.get(), &c.color.r, &c.color.g, &c.color.b, &c.color.a);
		return c;
	}
...
private:
	const std::unique_ptr<SDL_Renderer, sdl2::Deleter<&SDL_DestroyRenderer>> impl_;
};
...
void render(Renderer& r, const Triangle& t) {
	...
	auto in = t;
	static const auto do_copy = [](SDL_Color& to, Color from) { memcpy(&to, &from.color, sizeof(Color)); };
...
std::enable_if_t<!std::is_arithmetic_v<Shape>, Shape> random(int windowWidth, int windowHeight);
...
sdl2::Triangle random<sdl2::Triangle>(int windowWidth, int windowHeight) {
	SDL_Vertex v1, v2, v3;
	v1.position = { random(0.f, 0.f + windowWidth), random(0.f, 0.f + windowHeight) };
	v2.position = { random(0.f, 0.f + windowWidth), random(0.f, 0.f + windowHeight) };
	v3.position = { random(0.f, 0.f + windowWidth), random(0.f, 0.f + windowHeight) };
	return { v1, v2, v3 };
}
...
template <> sdl2::Color random() {
	return { util::random<Uint8>(0, 255), util::random<Uint8>(0, 255), util::random<Uint8>(0, 255), util::random<Uint8>(0, 255) };
}
		...
		while (SDL_PollEvent(&event)) {
			...
			if (event.type == SDL_KEYDOWN) {
				switch (event.key.keysym.sym) {
					case SDLK_s:
						shapes.emplace_back(util::random<sdl2::Sphere>(windowWidth, windowHeight), util::random<Color>());
						break;
					case SDLK_r:
						shapes.emplace_back(util::random<sdl2::Rectangle>(windowWidth, windowHeight), util::random<Color>());
						break;
					case SDLK_t:
						shapes.emplace_back(util::random<sdl2::Triangle>(windowWidth, windowHeight), util::random<Color>());
						break;
				}
			}
		}
